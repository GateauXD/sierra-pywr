...
ax = sns.barplot(overall_climate.index, overall_climate.values)
climate_scenarios.append("Historic")

plt.xticks(np.arange(len(climate_scenarios)), climate_scenarios ,fontsize=35, rotation=45)
plt.setp(ax.spines.values(), linewidth=3)
plt.yticks(fontsize="54")
plt.title("Lake McClure Inflow No. of flood days annually " + time[index], fontsize=60)
plt.ylabel("No. of days", fontsize='54')
plt.savefig("Figures/30_lake_mcclure_inflow_averages_" + time[index].split()[0])

# ax = yearly_delta_change.reset_index().plot(x="index", y=climate_scenarios, kind="bar")
# plt.xticks(rotation=0, fontsize="54")
# plt.yticks(fontsize="54")
# plt.setp(ax.spines.values(), linewidth=3)
...
#
# ax = yearly_percent_change.reset_index().plot(x="index", y=climate_scenarios, kind="bar")
# plt.xticks(rotation=0, fontsize="54")
# plt.setp(ax.spines.values(), linewidth=3)
# plt.yticks(fontsize="54")
